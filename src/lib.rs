...
mod tests{
    ...
    fn test_is_pronic(){
        for i in &factor::TEST_PRONIC {
            ...
            assert!(!factor::is_pronic(8));
            assert!(!factor::is_pronic(10));
    }

    #[test]
    fn test_is_squarefree(){
        for i in &factor::TEST_SQUAREFREE {
            assert!(factor::is_squarefree(*i));
        }
            assert!(!factor::is_squarefree(8));
            assert!(!factor::is_squarefree(9));
    }

    #[test]
    fn test_is_sphenic(){
        for i in &factor::TEST_SPHENIC{
        assert!(factor::is_sphenic(*i));
    }
        assert!(!factor::is_sphenic(19));
        assert!(factor::is_sphenic(786));
        assert!(!factor::is_sphenic(788));
        assert!(!factor::is_sphenic(72));   
    }

    #[test]
    fn test_is_semiprime(){
        for &i in &factor::TEST_SEMIPRIME {
        assert!(factor::is_semiprime(i));}
        assert!(!factor::is_semiprime(19));
        assert!(!factor::is_semiprime(786));
        assert!(!factor::is_semiprime(-72));
    }   

    #[test]
    fn test_is_achilles(){
        for i in &factor::TEST_ACHILLES {
            assert!(factor::is_achilles(*i));
        }
        assert!(!factor::is_achilles(360));
        assert!(!factor::is_achilles(784));
        assert!(!factor::is_achilles(-72));
    }

    #[test]
    fn test_is_powerful(){
        for i in &factor::TEST_POWERFUL {
            assert!(factor::is_powerful(*i));
        }
        assert!(!factor::is_powerful(19));
        assert!(!factor::is_powerful(-20));
        assert!(!factor::is_powerful(-25));
    }
